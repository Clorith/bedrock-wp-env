 ...
 * @param {string}   environment The environment to build the Dockerfile for.
 *
 * @return {string} The dockerfile contents.
 */
function dockerFileContents( image, config, environment = 'development' ) {
	...
	if ( config.xdebug !== 'off' ) {
		...
		if ( usingCompatiblePhp ) {
			...
			spinner.warn(
				`Warning: The current PHP version is not compatible with xDebug, the feature will not be enabled..`
			)
		}
	}
...
${ changeWebserverRootPath( config, environment ) }
${ shouldInstallXdebug ? installXdebug( config.xdebug ) : '' }
`;
}

function changeWebserverRootPath( config, environment ) {
	return `
# Set web root path
RUN sed -i "s,/var/www/html,/var/www/html/${ config.env[ environment ].publicDirectory },g" /etc/apache2/sites-available/000-default.conf
	`;
}
