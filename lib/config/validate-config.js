...
function validateConfig( config, envLocation ) {
	...
	if ( null === config.publicDirectory ) {
		throw new ValidationError(
			`No valid public directory was found, if you are using a custom location, please use the "publicDirectory" property.`
		);
	}

	if ( ! Array.isArray( config.postSetupCommands ) ) {
		throw new ValidationError(
			`Invalid .wp-env.json: "postSetupCommands" must be an array of strings.`
		);
	}
